...
async function getDashboardData() {
  ...
  return { projectCount, publishedCount, messageCount, unreadCount, recentMessages }
}
...
export default async function AdminDashboardPage() {
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: MessageSquare,
      href: '/admin/messages',
      color: unreadCount > 0 ? 'from-amber-500 to-orange-500' : 'from-emerald-500 to-teal-500',
    },
    {
      label: 'Live Site',
      value: 'View',
      ...
      <div className="max-w-5xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
          {stats.map((stat) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-xl bg-gradient-to-br ${stat.color} flex items-center justify-center mb-4 shadow-lg`}
                >
        ...
        <div className="mb-10">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link
              href="/admin/projects"
              className="inline-flex items-center gap-2 px-4 py-2.5 border border-border text-foreground text-sm font-semibold rounded-xl hover:bg-muted transition-all"
            >
            ...
            >
              ...
              Messages
              {unreadCount > 0 && (
                <span className="px-1.5 py-0.5 rounded-full text-xs bg-amber-500/20 text-amber-400 border border-amber-500/30">
                  {unreadCount}
                </span>
              )}
            </Link>
          ...
          <div>
            ...
            <div className="space-y-2">
              {recentMessages.map((msg) => (
                <div
                  key={msg.id}
                  className={`p-4 rounded-xl border ${!msg.read ? 'border-blue-500/20 bg-blue-500/5' : 'border-border bg-card'}`}
                >
